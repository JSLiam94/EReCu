...
from erecu.utils import load_yaml, safe_logit
...
def predict(
    ...
    for scale in (0.5, 0.75, 1.0, 1.25, 1.5):
        size = tuple(_round_to_patch(dim * scale, patch_size) for dim in base_size)
        ...
        for flipped in (False, True):
            ...
            logit_views.append(safe_logit(prediction))
...
def main() -> None:
    ...
    parser.add_argument("--dino-model", default=None, help="override model.backbone with a local DINOv2 .pth file or model directory")
    ...
    for batch in tqdm(loader, desc="Infer"):
        ...
        predictions = predict(model, image, native, model.backbone.patch_size)
