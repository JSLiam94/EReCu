...
class FrozenResNet18(nn.Module):
    def __init__(self, pretrained: bool = False, weights_path: str | None = None) -> None:
        super().__init__()
        if pretrained:
            if not weights_path:
                raise RuntimeError(
                    "ImageNet-pretrained ResNet-18 requires model.resnet_weights. "
                    "Run train.py so its preflight step downloads it to weights/."
                )
            path = Path(weights_path).expanduser()
            if not path.is_file() or path.stat().st_size == 0:
                raise FileNotFoundError(
                    f"ImageNet ResNet-18 weights are required but unavailable: {path}. "
                    "Training must not fall back to random ResNet weights."
                )
            try:
                ...
                raise RuntimeError(f"Could not load ImageNet ResNet-18 weights from {path}") from exc
        else:
...
class MultiCueNativePerception(nn.Module):
    ...
    def __init__(
        ...
        self.semantic = FrozenResNet18(resnet_pretrained, resnet_weights_path)
